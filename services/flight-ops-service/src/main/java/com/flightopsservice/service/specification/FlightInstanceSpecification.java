package com.flightopsservice.service.specification;
...
public class FlightInstanceSpecification {
    ...
    public static Specification<FlightInstance> buildSearchSpec(
        ...
        return ((root, query,
            ...
            //2 exclude terminal statues
            predicates.add(root.get("status").in(EXCLUDED_STATUSES).not());

            //3flight must still be in the future
            ...
            predicates.add(cb.between(
                    root.get("departureDateTime"),startOfDate,endOfDay));

            //euals o less than toatl passegers
            ...
            if (request.getMaxDuration() !=null){
                ...
                predicates.add(cb.lessThanOrEqualTo(durationMinutes,
    ...
    private static void applyTimeRangePredicate(
            List<Predicate> predicates, Root<FlightInstance> root,
            CriteriaBuilder cb, String dateTimeField,String timeRange){
        Expression<Integer> hour=cb.function("HOUR",Integer.class,
