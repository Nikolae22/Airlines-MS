...
public class FareRulesMapper {
    ...
    public static FareRulesResponse toDTO(FareRules fareRules){
        if (fareRules == null) return null;
        return FareRulesResponse.builder()
                .id(fareRules.getId())
                .ruleName(fareRules.getRoleName())
    ...
    public static void updateEntity(FareRulesRequest request, FareRules existing){
        ...
        if (request.getRuleName() !=null) existing.setRoleName(request.getRuleName());
